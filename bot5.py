...
def upsert_user(user_id, name):
    ...
    VALUES (?, ?, ?, 0)
    """, (user_id, name, str(now())))

    cursor.execute("""
    UPDATE users
    SET name=?, last_seen=?
    WHERE user_id=?
    """, (name, str(now()), user_id))
...
async def check_users(app: Application):
    ...
    for user_id, name, last_seen, warned in users:
        ...
        # DAY 1 WARNING
        # =====================
        if diff.days >= 1 and warned == 0:
            try:
                await app.bot.send_message(
                    chat_id=GROUP_ID,
                    text=f"⚠️ {name}, please send a photo of your spender today or you will be removed tomorrow."
                )
                set_warned(user_id)
            except:
                pass
